...
from src.models.sdad.dictionary import Dictionary
from src.models.sdad.organization import Organization
from src.models.sdad.organizational_unit import OrganizationalUnit

from bson import json_util

sdad = Blueprint("sdad", __name__)

# Dictionary Routes

# @sdad.route("/dictionary/<string:dictionary>/<int:id>/description")
# def get_dictionary_id(dictionary: str, id: int):
#   try:
#     doc = Dictionary.objects().get(code=dictionary, sdad_id=id)
...
# @sdad.route( "/dictionary/<string:dictionary>/<string:description>/id" )  # fmt: skip
...
# @sdad.route("/dictionary/<string:dictionary>")
# def get_dictionary(dictionary: str):
#   dictionary = Dictionary.objects(code=dictionary).only("sdad_id", "description").exclude("id")
#   return Response(dictionary.to_json(), mimetype="application/json", status=200)


# @sdad.route("/dictionary/<string:dictionary>/ids")
# def get_dictionary_ids(dictionary: str):
#   docs = Dictionary.objects(code=dictionary)
#   ids = [doc["sdad_id"] for doc in docs]
#   return Response(json.dumps(ids), mimetype="application/json", status=200)


# Organization Routes
@sdad.route("/organization/all/pagination")
def get_all_organizations_with_pagination():
  try:
    # Pagination
    ...
    for field, condition in filters.items():
      field = field.split(".")[0] + "__" + field.split(".")[1] if field in ["organizationCode.preferredLabel", "subOrganizationOf.preferredLabel"] else field
    ...
    total = Organization.objects(**query).count()
...
# @sdad.route("/organization/all")
# def get_all_organizations():
#   data = (
#     Organization.objects.only("code", "preferredLabel", "subOrganizationOf", "organizationType", "status", "foundationFek", "mainAddress")
#     .exclude("id")
#     .order_by("preferredLabel")
#   )
#   return Response(
...
# @sdad.route("/organization/<string:code>")
...
# @sdad.route("/organization/<string:label>/label")
...
# @sdad.route("/organizationalUnit")
# def get_organization_unit():
#   organization = OrganizationalUnit.objects().only("code", "preferredLabel").exclude("id")
#   return Response(organization.to_json(), mimetype="application/json", status=200)

# @sdad.route("/organizationalUnit/all")
# def get_all_organizational_units():
#     data = (
#         OrganizationalUnit.objects.only("code", "organizationCode", "supervisorUnitCode", "preferredLabel", "unitType", "supervisorUnitCode","email", "telephone", "mainAddress", "remitsFinalized")
#         .exclude("id")
#         .order_by("preferredLabel")
#     )
...
@sdad.route("/organizationalUnit/all/pagination")
...
# @sdad.route("/organizationalUnit/count")
...
@sdad.route("/organizationalUnit/<string:code>")
...
@sdad.route("/organizationalUnit/<string:code>/organizationCode")
